...
void LoadParameters( const std::string filename, int &L, int &OBC, int &N,
  ...
  RealType &dt, int &Tsteps, RealType &Gamma);
...
int main(int argc, char const *argv[]) {
  ...
  LoadParameters( "conf.h5", L, OBC, N, Uin, Veqm, Vdyn, dt, Tsteps, gamma );
  INFO("Build Lattice - ");
  ...
  for (size_t cntT = 1; cntT <= Tsteps; cntT++) {
    Lindblad_RK4( dt, gamma, Rho0, ham, Rho );
