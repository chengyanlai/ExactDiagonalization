...
ComplexVectorType CoherentState(const std::vector<ComplexType>& alphas, const std::vector<Basis>& Bases, const Holstein<ComplexType>& Ham0){
  ...
  // std::cout << Eig[0] << " " << Eig[1] << " " << Eig[2] << std::endl;
  // std::cout << EigVec[0] << " " << EigVec[1] << " " << EigVec[2] << std::endl;
...
void Dynamics(const std::string prefix, const std::string InitialState, const int S1, const int S2, const int MeasureEvery, const int SaveWFEvery ){
  ...
  if ( InitialState == "E" && S1 >= 0 && S2 >= 0 ){
    ...
    VecInit = CoherentState(alphas, Bases, Ham0);
    LogOut << " DONE!" << std::endl;
  }else{
    SaveFile.append( "-R" );
