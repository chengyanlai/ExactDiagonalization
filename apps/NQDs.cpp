...
void Dynamics( const std::string prefix, const int SearchJ = 0 ){
  ...
  w1.clear();
  w2.clear();
  LogOut << "\tC_1,up" << std::endl;
  Cf(1, 0, Bases, Hams, PairIndex1, PairIndex2, w1, w2);
  // GammaW.push_back(Gammas.at(0));
  SiteTypesSpin.push_back(std::make_tuple(1,-1, 0));
  BasisIds.push_back(w1);
  CollapseIds.push_back(w2);
  w1.clear();
  w2.clear();
  LogOut << "\tC_1,dn" << std::endl;
  Cf(1, 1, Bases, Hams, PairIndex1, PairIndex2, w1, w2);
  // GammaW.push_back(Gammas.at(0));
  SiteTypesSpin.push_back(std::make_tuple(1,-1, 1));
  BasisIds.push_back(w1);
  CollapseIds.push_back(w2);
  LogOut << "DONE!" << std::endl;
